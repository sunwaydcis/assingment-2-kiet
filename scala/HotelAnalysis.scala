...
case class Booking(
  ...
  // Optimization: Calculate these once when needed (Lazy evaluation)
  // This demonstrates "Elegant application of language features"
...
object HotelAnalysis extends App {
  ...
  // Using ISO-8859-1 to handle special characters correctly
  ...
  val bookings: List[Booking] = try {
      ...
      .flatMap { line =>
        if (line.trim.isEmpty) None
          ...
          if (cols.length < 24) None // Skip malformed lines
            ...
            // "Try" handles parsing errors gracefully (Safety)
            Try {
              Booking(
                bookingId = cols(0),
    ...
    source.close() // Best Practice: Always close resources
  }
  ...
  // --- ANALYSIS 1: HIGHEST BOOKINGS (OPTIMIZED) ---
  // Using foldLeft instead of groupBy for memory efficiency (O(n) pass)
  ...
  // --- ANALYSIS 2: MOST ECONOMICAL HOTEL (ELEGANT) ---
  // Using the lazy val 'netPrice' we defined in the case class
  ...
  // --- ANALYSIS 3: MOST PROFITABLE HOTEL (OPTIMIZED) ---
  // Using foldLeft to accumulate profit directly without creating intermediate lists
